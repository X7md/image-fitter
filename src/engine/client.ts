...
export class EngineClient {
  ...
  /** Hand a source image to the engine under `key` (never reuse a key). The pixel buffer
   *  is transferred, so the caller must not use `bitmap.data` afterwards. */
  addImage(key: number, bitmap: Bitmap): Promise<void> {
    const buffer = bitmap.data.buffer
    return this.request({ type: 'addImage', id: 0, key, bitmap }, buffer instanceof ArrayBuffer ? [buffer] : []).then(() => undefined)
  }

  removeImage(key: number): Promise<void> {
    return this.request({ type: 'removeImage', id: 0, key }).then(() => undefined)
  }

  clearImages(): Promise<void> {
    return this.request({ type: 'clearImages', id: 0 }).then(() => undefined)
  }

  /** Stack (optional) and fit. 'preview' renders at a capped size. */
  async render(job: RenderJob, quality: Quality = 'full'): Promise<Bitmap> {
    const bitmap = await this.request({ type: 'render', id: 0, job, quality })
