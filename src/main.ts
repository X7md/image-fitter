...
function calculateAspectRatio(
  ...
  switch (ratio) {
    ...
    case "16:9":
      ...
      return option1_16_9.width * option1_16_9.height >
        option2_16_9.width * option2_16_9.height
        ? option1_16_9
        : option2_16_9;
    case "4:3":
      // Calculate both possibilities and choose the one with larger area
      const option1_4_3 = {
        width: (currentHeight * 4) / 3,
        height: currentHeight,
      };
      const option2_4_3 = {
        width: currentWidth,
        height: (currentWidth * 3) / 4,
      };
      return option1_4_3.width * option1_4_3.height >
        option2_4_3.width * option2_4_3.height
        ? option1_4_3
        : option2_4_3;
    case "3:2":
      // Calculate both possibilities and choose the one with larger area
      const option1_3_2 = {
        width: (currentHeight * 3) / 2,
        height: currentHeight,
      };
      const option2_3_2 = {
        width: currentWidth,
        height: (currentWidth * 2) / 3,
      };
      return option1_3_2.width * option1_3_2.height >
        option2_3_2.width * option2_3_2.height
        ? option1_3_2
        : option2_3_2;
    default:
...
function setupEventListeners() {
  ...
  document.querySelectorAll(".arrow-btn").forEach((btn) => {
    ...
    btn.addEventListener("mousedown", startContinuousMovement);
  ...
  document.querySelectorAll(".bg-option-btn").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      // Update active state
      document
        .querySelectorAll(".bg-option-btn")
        .forEach((b) => b.classList.remove("active"));
      target.classList.add("active");
      ...
      if (state.backgroundMode === "blur") {
        ...
        tempCanvas.toBlob(async (blob) => {
          ...
          try {
            ...
            const resultBlob = new Blob([new Uint8Array(processedData)], {
              type: "image/png",
            });
            ...
            console.error(
              "ImageMagick processing failed, falling back to canvas:",
              magickError
            );
  ...
  elements.resetBtn.addEventListener("click", () => {
    ...
    document
      ...
      .forEach((b) => b.classList.remove("active"));
    document
      .querySelector('.aspect-btn[data-ratio="custom"]')
    ...
    document
      ...
      .forEach((b) => b.classList.remove("active"));
    document
      .querySelector('.align-btn[data-align="center"]')
      ?.classList.add("active");
    ...
    document
      ...
      .forEach((b) => b.classList.remove("active"));
    document
      .querySelector('.bg-option-btn[data-bg="color"]')
      ?.classList.add("active");
...
function init() {
  ...
  elements.blurControls.style.display =
    state.backgroundMode === "blur" ? "block" : "none";
